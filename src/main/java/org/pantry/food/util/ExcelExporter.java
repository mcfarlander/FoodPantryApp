...
/**
 * Exports a TableView to an Excel spreadsheet
 *
...
public class ExcelExporter<T> {
	...
	public void export(TableView<T> tableView, File outputFile, String sheetName) throws IOException {
		...
		HSSFSheet hssfSheet = hssfWorkbook.createSheet(null == sheetName ? "Sheet 1" : sheetName);
		...
		for (int row = 0; row < tableView.getItems().size(); row++) {
			...
			for (int col = 0; col < tableView.getColumns().size(); col++) {
				...
				// Looks weird in LibreOffice
//				T entity = tableView.getItems().get(row);
//				if (entity instanceof ReportRow && ((ReportRow) entity).isSummary()) {
